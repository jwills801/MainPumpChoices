...
Accumulator_size1_actual + Accumulator_size2_actual
...
for i = 1:length(a)
    perc_threshold = a(i);
    K = b(i);
    Accumulator_Size(i) = max([-2*(1+perc_threshold)^(1/K)/(1-(1+perc_threshold)^(1/K))*delV_max1 , -2*(1-perc_threshold)^(1/K)/(1-(1-perc_threshold)^(1/K))*delV_min1]) ;
end
