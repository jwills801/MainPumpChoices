...
Q = 5*Qave; % Arbitraily set flow rate - to be changed later
Cost2Switch = 1e-1; Alos arbitraily set - to be changed later

% nn is the number of time steps with flow it takes to get the the required
% volume
nn = max(ceil(V1(end)/Q/DPdt) +2);
...
for k = 1:length(DPt)-1
    ...
    for j = 1:nn^size(V,2)-1
        if X(j+1) == max(X) % The case where rail 1 should not be filled anymore
            [J(j+1,end-k),ind(j+1,end-k)] = min([J(j+1,end-k+1)+Cost2Switch*(ind(j+1,end-k+1)==3 | ind(j+1,end-k+1)==2),inf,J(j,end-k+1)+Cost2Switch*(ind(j+1,end-k+1)==1 | ind(j+1,end-k+1)==2)]);
        ...
        elseif Y(j+1) == max (Y) % The case where rail 2 should not be filled anymore
            [J(j+1,end-k),ind(j+1,end-k)] = min([J(j+1,end-k+1)+Cost2Switch*(ind(j+1,end-k+1)==3 | ind(j+1,end-k+1)==2),J(j+1-length(V_MP1),end-k+1)+Cost2Switch*(ind(j+1,end-k+1)==1 | ind(j+1,end-k+1)==3),inf]);
            J(j+1,end-k) = J(j+1,end-k) + abs(V(t_ind,1)-V_MP(X(j+1))) + abs(V(t_ind,2)-V_MP(Y(j+1)));            
            
        else
            [J(j+1,end-k),ind(j+1,end-k)] = min([J(j+1,end-k+1)+Cost2Switch*(ind(j+1,end-k+1)==3 | ind(j+1,end-k+1)==2),J(j+1-length(V_MP1),end-k+1)+Cost2Switch*(ind(j+1,end-k+1)==1 | ind(j+1,end-k+1)==3),J(j,end-k+1)+Cost2Switch*(ind(j+1,end-k+1)==1 | ind(j+1,end-k+1)==2)]);
            J(j+1,end-k) = J(j+1,end-k) + abs(V(t_ind,1)-V_MP(X(j+1))) + abs(V(t_ind,2)-V_MP(Y(j+1))); 
...
for kk  = 2:length(DPt)
    ...
    CostCheck = CostCheck + (IsPumping(kk-1)==1)*Cost2Switch + (IsPumping(kk-1)==2)*Cost2Switch;
end
